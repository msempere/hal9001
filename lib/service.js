...
function Service(aws, bot, namespace, period, dimension, supported_metrics) {
    ...
    function displayMetric(channel, instance_id, metric){
        ...
        if(metric && supported_metrics.indexOf(metric) != -1){
            ...
            utils.displayMetric(aws.cloudwatch, bot, channel, namespace, metrics[metric].name, metrics[metric].unit, instance_id, dimension, period);
        }
        else{
            // display header
            bot.postMessage(channel, "*'" + instance_id + "'* for the last " + period + " minutes:", {as_user: true, mrkdwn: true});
            ...
            for (index = 0; index < supported_metrics.length; index++) {
                utils.displayMetric(aws.cloudwatch,
                                    bot,
                                    channel,
                                    namespace,
