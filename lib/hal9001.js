...
Hal9001.prototype._displayHelp = function (channel) {
    ...
    help_message += "\thal9001 *ec2* <id>\t metrics for AWS EC2 <id> instance\n";
    help_message += "\thal9001 *rds* <id>\t metrics for AWS RDS <id> instance\n";
    ...
    if(isHelp(message.text)){
        ...
        if(m && m.length >= 3){
            ...
            switch(m[1]){
                case('ec2'):
                    var ec2 = new EC2();
                    ec2.displayAllMetrics(this.cloudwatch,
                                       self,
                                       message.channel,
                                       m[2]);
                    break;
