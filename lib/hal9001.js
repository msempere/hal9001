...
var RDS = require('../lib/plugins/rds');
var EC2 = require('../lib/plugins/ec2');
...
Hal9001.prototype._loadPlugin = function (plugin) {
    // instantiate plugin and add it to bot plugins
    if(this.hasOwnProperty('plugins')){
        this.plugins.push(plugin(this.AWS_services, this));
    }
    else{
        this.plugins = [plugin(this.AWS_services, this)];
    }
    return this;
};
    ...
    if(m && m.length >= 3){
        ...
        for(index=0; index<this.plugins.length; index++){
            ...
            if(m[1] == plugin.getNamespace()){
                plugin.displayMetric(message.channel, m[2], m[3]);
                service_found = true;
            }
        }
        // service not recognized
        if(!service_found){
            this.postMessage(message.channel, "Service '" + m[1] + "' not loaded. Please contact my creator", {as_user: true});
        }
    }
    // input doesn't match regular expression
    else{
        this.postMessage(message.channel, "?", {as_user: true});
    }
};
